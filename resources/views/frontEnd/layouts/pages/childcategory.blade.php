...
@section('breadcrumb')<a href="{{ route('subcategory', optional($childcategory->subcategory)->slug) }}">{{ optional($childcategory->subcategory)->subcategoryName }}</a><span class="mx-2">›</span>{{ $childcategory->childcategoryName }}@endsection
@push('css')
<style>
...
<div class="fashion-container listing-wrap">
    <div class="listing-head">
        <div><h1>{{ $childcategory->childcategoryName }}</h1><span class="count">{{ $products->total() }} products found</span></div>
    ...
    <div class="listing-layout">
        <aside class="filter-card" id="filter-panel">
            <div class="filter-block"><h6>More Categories</h6>@foreach($childcategories as $child)<a href="{{ route('products',$child->slug) }}" class="@if($child->id==$childcategory->id) on @endif">{{ $child->childcategoryName }}</a>@endforeach</div>
