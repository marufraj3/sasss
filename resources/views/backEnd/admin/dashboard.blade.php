...
<div class="container-fluid">
    ...
    <div class="row g-3 mb-4">
        <div class="col-lg-4"><div class="card metric-card h-100"><div class="card-body"><h5 class="table-title">Quick actions</h5><div class="row g-2 mt-1"><div class="col-6"><a class="quick-action" href="{{ route('products.create') }}"><i class="fe-package d-block fs-4 mb-1"></i>Product add</a></div><div class="col-6"><a class="quick-action" href="{{ route('campaign.create') }}"><i class="fe-layout d-block fs-4 mb-1"></i>Landing page</a></div><div class="col-6"><a class="quick-action" href="{{ route('admin.orders', ['slug' => 'pending']) }}"><i class="fe-list d-block fs-4 mb-1"></i>Orders</a></div><div class="col-6"><a class="quick-action" href="{{ route('shippingcharges.index') }}"><i class="fe-truck d-block fs-4 mb-1"></i>Delivery charge</a></div></div><hr><div class="d-flex justify-content-between"><span class="small-muted">Total completed revenue</span><strong>৳{{ number_format($totalRevenue) }}</strong></div><div class="d-flex justify-content-between mt-2"><span class="small-muted">Total orders</span><strong>{{ number_format($total_order) }}</strong></div><div class="d-flex justify-content-between mt-2"><span class="small-muted">Products</span><strong>{{ number_format($total_product) }}</strong></div><hr><div class="d-flex justify-content-between align-items-center"><span class="small-muted"><i class="fe-alert-triangle text-warning me-1"></i>Low stock (≤ 5)</span><a href="{{ route('admin.stock_report') }}" class="fw-bold text-warning">{{ $lowStockCount }}</a></div>@if($lowStockProducts->isNotEmpty())<div class="mt-2">@foreach($lowStockProducts as $product)<div class="d-flex justify-content-between small py-1 border-bottom"><span>{{ Str::limit($product->name, 24) }}</span><strong class="text-danger">{{ $product->stock }}</strong></div>@endforeach</div>@endif</div></div></div>
